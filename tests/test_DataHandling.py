...
GlobalData = DataHandling.LoadData("testData.raw") # Load data to be used in upcoming tests - so that it doesn't need to be loaded for each individual function to be tested
...
def test_plotPSD():
    """
    This tests that the plot of the PSD produced by DataObject.plotPSD is produced correctly and matches the baseline to a certain tolerance.
    """
    fig, ax = GlobalData.plotPSD([0, 400e3], ShowFig=False)
    return fig

@pytest.mark.mpl_image_compare(tolerance=20) # this decorator compares the figure object returned by the following function to the baseline png image stored in tests/baseline
def test_getFit():
    """
    Tests that DataObject.getFit works and therefore tests fitPSD, fit_curvefit and PSD_Fitting as these are dependancies. It tests that the output values of the fitting are correct (both the values and thier errors) and that the plot looks the same as the baseline, within a certain tolerance.
    """
    A, F, Gamma, fig, ax = GlobalData.getFit(10000, 1, 75000, ShowFig=False)
...
def test_ExtractParameters():
    """
    Tests that DataObject.ExtractParameters works and returns the correct values.
    """
    with open("testDataPressure.dat", 'r') as file:
        for line in file:
            pressure = float(line.split("mbar")[0])
    R, M, ConvFactor = GlobalData.ExtractParameters(pressure, 0.15)
